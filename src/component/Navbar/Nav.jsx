...
const Nav = () => {
  ...
  return (
    ...
    <nav className="flex justify-between max-w-container mx-auto py-4 ">
      ...
      <button className="button-me md:py-2 md:px-5 py-2 px-5 font-semibold text-base overflow-hidden  border-primary-color border-2 rounded-full md:rounded-lg after:bg-primary-color text-button-text after:z-[-10] hover:text-primary-color  z-50">Resume</button>
      </ul>
      {/* Mobile Nav */}
      <ul className={`fixed text-[#EDEDED] text-center items-center text-lg top-0 z-50 bg-[#112e42] w-2/3 h-screen shadow-2xl
      md:hidden flex flex-col gap-10 text-medium  p-7 pt-20 duration-500
      ${open ? "right-0" : "right-[-100%]"}`} >

         {Links.map((link, i) => (
        <li key={i} className="font-semibold hover:text-primary-color">
          <NavLink  onClick={closeMobileMenu} style={({ isActive, isPending }) => {
            return {
              color: isActive ? "#00ABF0" : "inherit",
            };
          }}
